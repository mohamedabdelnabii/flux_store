...
class HtmlService {
  ...
  static Widget getHtmlWidget(String htmlData) {
    // Remove "More products" links before displaying
    String cleanedHtml = _removeMoreProductsLinks(htmlData);
    return Html(data: cleanedHtml, style: _getHtmlStyles());
  }

  /// Get HTML styles configuration
  static Map<String, Style> _getHtmlStyles() {
    return {
      "body": Style(
        margin: Margins.zero,
      ...
      "blockquote": Style(
        ...
        border: Border(left: BorderSide(color: AppColors.grey, width: 2)),
  ...
  static Map<String, Style> getHtmlStylesWithFontSize(double fontSize) {
    final styles = _getHtmlStyles();
  ...
  static Widget getHtmlWidgetWithFontSize(String htmlData, double fontSize) {
    // Remove "More products" links before displaying
    String cleanedHtml = _removeMoreProductsLinks(htmlData);
    return Html(data: cleanedHtml, style: getHtmlStylesWithFontSize(fontSize));
  }
